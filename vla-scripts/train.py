...
from prismatic.vla import get_vla_dataset_and_collator, get_cotraining_datasets
...
class TrainConfig:
    ...
    local_rank: Optional[int] = 0                                # Local Rank (for torch.distributed)
    vla: VLAConfig = field(
    ...
    co_training: bool = True                                        # Enable Co-Training
...
def train(cfg) -> None:
    ...
    overwatch.info(f"local_rank: {cfg.local_rank}")
    overwatch.info(f"rank: {overwatch.rank()}")
    overwatch.info(f"world_size: {overwatch.world_size()}")
    
    # dist.barrier()
    overwatch.info("OpenVLA Training :: Ready to Go!")

    # Note => Under `torchrun` initializing `overwatch` will automatically set up `torch.distributed`
    torch.cuda.set_device(device_id := overwatch.local_rank())
    torch.cuda.empty_cache()

    if cfg.vla.enable_tf32:    
        torch.backends.cuda.matmul.allow_tf32 = True
        torch.backends.cudnn.allow_tf32 = True
        overwatch.info("Use TF32!")
    ...
    hf_token = Path(cfg.hf_token).read_text().strip() # if isinstance(cfg.hf_token, Path) else os.environ[cfg.hf_token]
    ...
    if cfg.co_training:
        overwatch.info(f"Loading Cotraining Datasets...")
        cotraining_datasets, cotraining_dataset_names = get_cotraining_datasets(
            cfg.data_root_dir,
            cfg.vla.co_training_data_mix,
            image_transform=vlm.vision_backbone.get_image_transform(),
            tokenizer=vlm.llm_backbone.get_tokenizer(),
            prompt_builder_fn=vlm.llm_backbone.prompt_builder_fn,
            shuffle_buffer_size=cfg.vla.shuffle_buffer_size,
        )
        print(f"Cotraining Datasets: {cotraining_dataset_names}")

        # Run VLA Training
        overwatch.info("Starting VLA Training Loop")
        train_strategy.run_vla_cotraining(
            vla_dataset,
            cotraining_datasets,
            cotraining_dataset_names,
            collator,
            action_tokenizer,
            metrics,
...
if __name__ == "__main__":
    parser = ArgumentParser(config_class=TrainConfig)
    parser.parser.add_argument("--local-rank", type=int)
