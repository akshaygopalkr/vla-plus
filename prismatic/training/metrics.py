...
COTRAINING_DATASETS = ["llava_data_filtered", "vqasynth_spacellava_filtered", "llava_onevision_data_filtered",
                       "robopoint_data"]
...
class WeightsBiasesTracker:
    ...
    def initialize(self) -> None:
        print(self.run_id, self.wandb_dir, self.hparams, self.project, self.entity, self.group)
        wandb.init(
            name=self.run_id,
            dir=self.wandb_dir,
            config=self.hparams,
        ...
        wandb.alert(title="Training OpenVLA", text=f"Project: {self.project}\nRun: {self.run_id}\n")
...
class VLAMetrics:
    ...
    def push(self) -> str:
        ...
        for ds, tracker in self.dataset_trackers.items():
            ...
            if ds not in COTRAINING_DATASETS:
                dataset_metrics.update(
                    {
                        f"{ds}/Loss": torch.stack(list(tracker.state["loss"])).mean().item(),
                ...
                dataset_metrics.update(
                    {
                        f"{ds}/Token Accuracy": torch.stack(list(tracker.state["action_accuracy"])).mean().item(),
                        f"{ds}/Loss": torch.stack(list(tracker.state["loss"])).mean().item()
                    }
                )
